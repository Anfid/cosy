...
function battery_widget:new(args)
    ...
    local widgets = { layout = wibox.layout.fixed.vertical }
    local batteries, mains, usb, usp = self:discover()
    local ac = mains[1] or usb[1] or usp[1]
...
function battery_widget:draw(context, cr, width, height)
    ...
    cr:rectangle(10.5, 0.5, 25, height - 1)
    cr:stroke()

    cr:move_to()
end

function battery_widget:fit(context, width, height) return width, 100 end
